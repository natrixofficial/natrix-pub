...
describe("Hasher", () => {
	test("Check `sha256Hex` with string", () => {
		...
		expect(hash).toBe("532eaabd9574880dbf76b9b8cc00832c20a6ec113d682299550d7a6e0f345e25");
	});

	test("Check `sha256Buffer` with string", () => {
		const hash = Hasher.sha256Buffer(Buffer.from("Test", "utf8"));
		expect(hash.toString("hex")).toBe("532eaabd9574880dbf76b9b8cc00832c20a6ec113d682299550d7a6e0f345e25");
	});
	...
	test("Check `hmacSha512Buffer` with string", () => {
		const hmac = Hasher.hmacSha512Buffer(Buffer.from("TestKey", "utf8"), Buffer.from("TestData", "utf8"));
	...
	test("Check `hmacSha256Hex` with string", () => {
		...
		expect(hmac).toBe("f8125d567c7a2a70a0e4c0197ea4f5b790ba1ebfa5dad1865579f88ba935340e");
	});

	test("Check `hmacSha256Buffer` with buffer", () => {
		const hmac = Hasher.hmacSha256Buffer(Buffer.from("TestKey", "utf8"), Buffer.from("TestData", "utf8"));
		expect(hmac.toString("hex")).toBe("f8125d567c7a2a70a0e4c0197ea4f5b790ba1ebfa5dad1865579f88ba935340e");
	});

	test("Check `createHmacSha256` with string", () => {
		const hmac = Hasher.createHmacSha256("TestKey", "utf8");
		hmac.update("TestData");
		expect(hmac.digest("hex")).toBe("f8125d567c7a2a70a0e4c0197ea4f5b790ba1ebfa5dad1865579f88ba935340e");
	});
});
