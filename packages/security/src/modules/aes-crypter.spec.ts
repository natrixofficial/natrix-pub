...
describe("AesCrypter", () => {
	...
	test("Testing string encryption and decryption with AesCrypter", () => {
		const aesKey = AesCrypter.generateAESKeyHex("Password");
		const encryptedData = AesCrypter.encryptAES256CbcHex("Test Data", aesKey.iv, aesKey.key);
		const decryptedData = AesCrypter.decryptAES256CbcHex(encryptedData, aesKey.iv, aesKey.key).toString("utf8");
	...
	test("Testing buffer encryption and decryption with AesCrypter", () => {
		...
		const encryptedData = AesCrypter.encryptAES256CbcBuffer(data, aesKey.iv, aesKey.key);
		const decryptedData = AesCrypter.decryptAES256CbcBuffer(encryptedData, aesKey.iv, aesKey.key);
	...
	test("Testing string encryption and decryption with wrong iv length with AesCrypter", () => {
		function errorTest(): void {
			...
			const data = Buffer.from("Test Data", "utf8");
			const encryptedData = AesCrypter.encryptAES256CbcBuffer(data, aesKey.iv, aesKey.key);
			AesCrypter.decryptAES256CbcBuffer(encryptedData, "iv1234567890", aesKey.key);
		}

		expect(errorTest).toThrowError(new Error("Invalid initialization vector"));
	});
	...
	test("Testing string encryption and decryption with wrong iv with AesCrypter", () => {
		function errorTest(): void {
			...
			const data = Buffer.from("Test Data", "utf8");
			const encryptedData = AesCrypter.encryptAES256CbcBuffer(data, aesKey.iv, aesKey.key);
			AesCrypter.decryptAES256CbcBuffer(encryptedData, "iv1234567890iviv", aesKey.key);
		}
