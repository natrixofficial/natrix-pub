...
import { IMember, IContribution, IReceivedContribution, IESH, IThresholdKey } from "../types/thresher.types";
...
export function PVSHEncodeG2(ID: Id, PK: PublicKey, sh: SecretKey, helperG2: G2): string {
	//Algorithm 1)
	const r = new Fr();
	r.setByCSPRNG();
	...
	const H = BLS.hashAndMapToG1(_appendBuffer(_appendBuffer(Q.serialize(), c.serialize()), U.serialize()));
...
export function PVSHVerifyG2(ID: Id, PK: PublicKey, PH: PublicKey, ESH: string, helperG2: G2): string {
...
export function PVSHDecodeG2(ID: Id, PK: PublicKey, SK: SecretKey, ESH: string): SecretKey {
	const ESHArray = ESH.split(".");
	if (ESHArray.length != 3) {
		throw new Error("Invalid ESH");
	}
...
function PVSHEncodeG1(ID: Id, PK: PublicKey, sh: SecretKey, helperG1: G1): string {
	//Algorithm 1)
	const r = new Fr();
	r.setByCSPRNG();
	...
	const H = BLS.hashAndMapToG2(_appendBuffer(_appendBuffer(Q.serialize(), c.serialize()), U.serialize()));
	//Algorithm 7)
	const V = BLS.mul(H, BLS.div(eh, r));
	H.clear();
	eh.clear();
...
function PVSHVerifyG1(ID: Id, PK: PublicKey, PH: PublicKey, ESH: string, helperG1: G1): string {
	const ESHArray = ESH.split(".");
	if (ESHArray.length != 3) {
		throw new Error("Invalid ESH");
	}
...
function PVSHDecodeG1(ID: Id, PK: PublicKey, SK: SecretKey, ESH: string): SecretKey {
	const ESHArray = ESH.split(".");
	if (ESHArray.length != 3) {
		throw new Error("Invalid ESH");
	}
...
class ThresherTypeGuards {
	...
	public static receivedContributionIsHexString(
		...
		if (receivedContributions.some((item) => typeof item.senderId !== "string")) {
			return false;
		}
		return true;
	}
	...
	public static receivedContributionIsBuffer(
		...
		if (receivedContributions.some((item) => !(item.senderId instanceof Buffer))) {
			return false;
		}
		return true;
	}
	...
	public static receivedContributionIsUint8Array(
		...
		if (receivedContributions.some((item) => !(item.senderId instanceof Uint8Array))) {
			return false;
		}
		return true;
	}
...
export class Thresher {
	private static calculateContributionInternal(
		...
		const helperG = BLS.isDefault() ? BLS.GetGeneratorOfPublicKey().to(G2) : BLS.GetGeneratorOfPublicKey().to(G1);
		const SG: SecretKey[] = [];
		const PG: PublicKey[] = [];
		...
		for (let i = 0; i < members.length; i++) {
			...
			ESH.push({ receiverId: members[i].id, receiverPK: members[i].pm, esh: ESH_k });
		}
	...
	private static calculateSharedKeysInternal(
		...
		for (let i = 0; i < shares.length; i++) {
			...
			for (let k = 0; k < ESHi.length; k++) {
				...
				if (reason) {
					errors.push({ senderId: shareItem.senderId, receiverId: ESHik.receiverId, reason: reason });
				}
				if (errors.length > 0) {
					continue;
				}
				...
				if (ESHik.receiverId.isEqual(meID)) {
					...
					//Algorithm 3)
					const SHik = BLS.isDefault() ? PVSHDecodeG2(IDik, mePK, meSK, ESHik.esh) : PVSHDecodeG1(IDik, mePK, meSK, ESHik.esh);
					//Algorithm 4)
		...
		for (let i = 0; i < PHForAll.length; i++) {
			...
			if (PHItem.SHk.length > 0) {
				...
				if (!PHItem.SH.getPublicKey().isEqual(PHItem.PH)) {
					errors.push({ senderId: null, receiverId: PHItem.ID, reason: "INVALID_SH_PH_FOR_ME" });
				}
			}
		}
		PG.recover(
	...
	public static calculateContribution(
		...
		if (ThresherTypeGuards.memberIsHexString(members)) {
			...
			const oldSHTemp = oldSH ? BLS.deserializeHexStrToSecretKey(oldSH as HexString) : undefined;
			return Thresher.calculateContributionInternal(threshold, membersTemp, oldSHTemp);
		}
		...
		if (ThresherTypeGuards.memberIsNative(members)) {
			return Thresher.calculateContributionInternal(threshold, members, oldSH as SecretKey);
		}
		throw new Error("generateContributionTTT:mismatch type");
	}

	public static contributionToHex(contribution: IContribution<Id, PublicKey>): IContribution<HexString, HexString> {
		//
	...
	public static contributionToBuffer(contribution: IContribution<Id, PublicKey>): IContribution<Buffer, Buffer> {
	...
	public static contributionToUint8Array(contribution: IContribution<Id, PublicKey>): IContribution<Uint8Array, Uint8Array> {
	...
	public static contributionFrom(contribution: IContribution<HexString, HexString>): IContribution<Id, PublicKey>;
	public static contributionFrom(contribution: IContribution<Buffer, Buffer>): IContribution<Id, PublicKey>;
	public static contributionFrom(contribution: IContribution<Uint8Array, Uint8Array>): IContribution<Id, PublicKey>;
	public static contributionFrom(contribution: IContribution<unknown, unknown>): IContribution<Id, PublicKey> {
		if (ThresherTypeGuards.contributionIsHexString(contribution)) {
			const result: IContribution<Id, PublicKey> = {
				pg: contribution.pg.map((item) => BLS.deserializeHexStrToPublicKey(item)),
				esh: contribution.esh.map((item) => {
					return {
	...
	public static calculateThresholdKeys(
		...
		if (
			ThresherTypeGuards.receivedContributionIsHexString(receivedContributions) &&
			typeof meID === "string" &&
			typeof meSK === "string"
		) {
			...
			return Thresher.calculateSharedKeysInternal(receivedContributionTemp, meIDTemp, meSKTemp);
		}
		if (ThresherTypeGuards.receivedContributionIsBuffer(receivedContributions) && meID instanceof Buffer && meSK instanceof Buffer) {
			const receivedContributionTemp = receivedContributions.map((item) => {
				const id = new Id();
		...
		if (
			ThresherTypeGuards.receivedContributionIsUint8Array(receivedContributions) &&
		...
		if (ThresherTypeGuards.receivedContributionIsNative(receivedContributions) && meID instanceof Id && meSK instanceof SecretKey) {
			return Thresher.calculateSharedKeysInternal(receivedContributions, meID, meSK);
		}
		throw new Error("generateContributionTTT:mismatch type");
	}

	public static thresholdKeyToHex(sharedKey: IThresholdKey<Id, SecretKey, PublicKey>): IThresholdKey<HexString, HexString, HexString> {
	...
	public static thresholdKeyToBuffer(sharedKey: IThresholdKey<Id, SecretKey, PublicKey>): IThresholdKey<Buffer, Buffer, Buffer> {
		const result: IThresholdKey<Buffer, Buffer, Buffer> = {
			...
			errors: sharedKey.errors.map((item) => {
				return {
					senderId: item.senderId ? Buffer.from(item.senderId.serialize()) : null,
					receiverId: Buffer.from(item.receiverId.serialize()),
					reason: item.reason,
				};
			}),
	...
	public static thresholdKeyFrom(thresholdKey: IThresholdKey<HexString, HexString, HexString>): IThresholdKey<Id, SecretKey, PublicKey>;
	public static thresholdKeyFrom(thresholdKey: IThresholdKey<Buffer, Buffer, Buffer>): IThresholdKey<Id, SecretKey, PublicKey>;
	public static thresholdKeyFrom(
		thresholdKey: IThresholdKey<Uint8Array, Uint8Array, Uint8Array>,
	): IThresholdKey<Id, SecretKey, PublicKey>;
	public static thresholdKeyFrom(thresholdKey: IThresholdKey<unknown, unknown, unknown>): IThresholdKey<Id, SecretKey, PublicKey> {
		if (ThresherTypeGuards.thresholdKeyIsHexString(thresholdKey)) {
			const result: IThresholdKey<Id, SecretKey, PublicKey> = {
				...
				errors: thresholdKey.errors.map((item) => {
					return {
						senderId: item.senderId ? BLS.deserializeHexStrToId(item.senderId) : null,
						receiverId: BLS.deserializeHexStrToId(item.receiverId),
						reason: item.reason,
					};
				}),
