import { BLS, Fr, G1, G2, SecretKey, PublicKey, GT, Signature } from "./bls-helper";
		...
		if (BLS.isDefault()) {
			...
			if (!prvData) {
				...
				prvData = {
					...
					chainAcc: BLS.mul(BLS.mul(helperG1, ledgerSK), BLS.inv(chainSK.to(Fr))).to(Signature),
			...
			if (!prvData) {
				...
				prvData = {
					...
					chainAcc: BLS.mul(BLS.mul(helperG2, ledgerSK), BLS.inv(chainSK.to(Fr))).to(Signature),
					ledgerChainPK: ledgerPK.to(PublicKey),
				};
			}
...
class ProofCreatorG1 implements IProofCreator {
	...
	public getBlockProof(): Block {
		if (!this.isCreated) {
			...
			this.chainAcc = BLS.mul(this.chainAcc, BLS.mul(this.blockHash, this.chainSK));
		}
		this.isCreated = true;
		return {
	...
	public getItemProof(dataIdx: number, hash: string): BlockItem {
		...
		const itemHashFr = BLS.hashToFr(hash);
		const blockProof = BLS.mul(this.blockAcc, BLS.add(itemHashFr, this.blockSK).inv());
		const chainProof = BLS.mul(this.chainAcc, BLS.add(itemHashFr, this.chainSK).inv());
		return {
			blockIdx: this.blockIndex,
			dataIdx: dataIdx,
...
function createBlockG1(
	...
	for (let i = 0; i < newBlock.items.length; i++) {
		...
		newBlock.blockAcc = BLS.mul(newBlock.blockAcc, BLS.add(newBlock.items[i].dataHashFr, blockSK));
	}
	...
	for (let i = 0; i < newBlock.items.length; i++) {
		newBlock.items[i].blockProof = BLS.mul(newBlock.blockAcc, BLS.add(newBlock.items[i].dataHashFr, blockSK).inv());
		newBlock.items[i].chainProof = BLS.mul(newBlock.chainAcc, BLS.add(newBlock.items[i].dataHashFr, chainSK).inv());
	}
...
function checkTrxG1(
	...
	//1: e(trxBlockProof, h*G2 + trxBlockPK)*e(-trxBlockAcc, G2) ?= 1 	//az adott H block proof-j oké-e (blockban van)
	const verify1_e1 = BLS.pairing(BLS.add(BLS.mul(helperG1, trxHash), trxBlockPK), trxBlockProof);
	...
	const verify2_e1 = BLS.pairing(BLS.add(BLS.mul(helperG1, trxHash), firstChainPk), trxChainProof);
	...
	const verify5_e1 = BLS.pairing(BLS.add(BLS.mul(deltaChainPk, trxHash), deltaNextChainPk), BLS.mul(trxChainProof, deltaChainHash));
...
function checkTrxG2(
	...
	//1: e(trxBlockProof, h*G2 + trxBlockPK)*e(-trxBlockAcc, G2) ?= 1 	//az adott H block proof-j oké-e (blockban van)
	const verify1_e1 = BLS.pairing(trxBlockProof, BLS.add(BLS.mul(helperG2, trxHash), trxBlockPK));
	...
	//2: e(trxChainProof, h*G2 + zeroChainPk)*e(-trxChainAcc, G2) 		//az adott H chain proof-ja oké-e ()
	const verify2_e1 = BLS.pairing(trxChainProof, BLS.add(BLS.mul(helperG2, trxHash), firstChainPk));
	...
	//ahol r[x] az act és a trx blockIdx különbségénél lévő chainPK értéke.
	const deltaChainHash = BLS.div(actChainHash, trxChainHash);
	const verify5_e1 = BLS.pairing(BLS.mul(trxChainProof, deltaChainHash), BLS.add(BLS.mul(deltaChainPk, trxHash), deltaNextChainPk));
